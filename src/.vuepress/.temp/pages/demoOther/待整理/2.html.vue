<template><div><h1 id="vue3-面试题" tabindex="-1"><a class="header-anchor" href="#vue3-面试题"><span>vue3 面试题</span></a></h1>
<h3 id="_1、vue3-的-composition-api-和-vue2-的-options-api-有什么区别" tabindex="-1"><a class="header-anchor" href="#_1、vue3-的-composition-api-和-vue2-的-options-api-有什么区别"><span>1、Vue3 的 Composition Api 和 Vue2 的 Options Api 有什么区别？</span></a></h3>
<table>
<thead>
<tr>
<th>类型</th>
...
<hr>
<h3 id="_2、vue3-的设计目标是什么-做了哪些优化" tabindex="-1"><a class="header-anchor" href="#_2、vue3-的设计目标是什么-做了哪些优化"><span>2、Vue3 的设计目标是什么，做了哪些优化？</span></a></h3>
<ol>
...
<ul>
<li>去掉不常用 api，体积更小</li>
<li>优化 diff 算法和响应式实现原理，响应更快</li>
<li>TypeScript 支持度更高</li>
<li>可维护性和自由度更高</li>
</ul>
<hr>
<h3 id="_3、vue3-性能提升主要是从哪几个方面体现" tabindex="-1"><a class="header-anchor" href="#_3、vue3-性能提升主要是从哪几个方面体现"><span>3、Vue3 性能提升主要是从哪几个方面体现</span></a></h3>
<ol>
<li><strong>diff 算法优化</strong>：新增静态节点标记，数据变化时，只对比动态节点。</li>
<li><strong>静态提升</strong>：静态节点只会被创建一次，数据变化时，免去了重新创建操作</li>
<li><strong>事件监听缓存</strong>：同上，数据变化时，免去重新绑定事件操作</li>
<li><strong>源码体积减小</strong>：去掉了不常用的 api，且所有函数都支持 Tree shanking，没用到的模块不会被打包</li>
<li><strong>响应式优化</strong>：用<code v-pre>proxy</code>重写响应式系统，对数据监听不需要深度遍历，可以监听动态添加的属性，可以监听数组索引和 length，可以监听删除属性等</li>
</ol>
<hr>
...
<ul>
<li><strong>defineProperty</strong>：是 Object 的方法，一次只能劫持一个属性，多个属性劫持需要遍历。如果是嵌套对象，则需要递归遍历。性能开销较大，而且对对象新增属性、数组通过索引修改数据、数组修改 length 无法监听到。虽然 vue2 对数组的<code v-pre>push</code>、<code v-pre>pop</code>、<code v-pre>shift</code>、<code v-pre>unshift</code>、<code v-pre>splice</code>、<code v-pre>sort</code>、<code v-pre>reverse</code> 7 个方法进行重写，但是其他数组 api 修改数组，依然无法监听到。</li>
<li><strong>Proxy</strong>：<code v-pre>Proxy</code>是对整个对象进行代理，而且是惰性的，只有当访问对象属性时才会触发，性能会更好。而且支持 13 种拦截方式，无需向 vue2 一样重写数组方法。</li>
</ul>
<hr>
<h3 id="_5、说说-vue3-的-tree-shaking-特性" tabindex="-1"><a class="header-anchor" href="#_5、说说-vue3-的-tree-shaking-特性"><span>5、说说 Vue3 的 Tree Shaking 特性</span></a></h3>
<ul>
<li><code v-pre>Tree Shaking</code>是一种通过清除多余代码，实现减少打包体积的技术。</li>
<li>vue3 中所有的 api 都是模块化的，用什么 api 就引入什么 api，没有用到的都不会打包。</li>
</ul>
<hr>
<h3 id="_6、说一下-vue3-新特性" tabindex="-1"><a class="header-anchor" href="#_6、说一下-vue3-新特性"><span>6、说一下 Vue3 新特性</span></a></h3>
<ol>
<li><strong>新增组合 api</strong>： <code v-pre>setup</code>、<code v-pre>ref</code>、<code v-pre>reactive</code>、<code v-pre>toRefs</code>、<code v-pre>toRef</code>等</li>
...
<li><strong>更好的 TS 支持</strong>: 源码是 TS 写的</li>
<li><strong>新增内置组件</strong>：<code v-pre>Fragment</code>(片段)、<code v-pre>Teleport</code>(瞬移)、<code v-pre>Suspense</code>(不确定)</li>
<li><strong>新的脚手架</strong>: vite (虽然我还在用 vue-cli5)</li>
<li><strong>多节点</strong>：一个组件可以设置多个根节点</li>
<li><strong>算法优化</strong>：重新 diff 算法、支持 tree-shaking，渲染速度更快、打包体积更小</li>
<li><strong>取出冗余</strong>：去掉了过滤器 filter，以及一些不常用的 api</li>
</ol>
<hr>
<h3 id="_7、说一下-reactive-和-ref-的区别" tabindex="-1"><a class="header-anchor" href="#_7、说一下-reactive-和-ref-的区别"><span>7、说一下 reactive 和 ref 的区别？</span></a></h3>
...
<li><code v-pre>reactive</code>：通过<code v-pre>Proxy</code>代理整个对象，深度拦截属性的读、写、删等操作（支持 13 种拦截类型），嵌套对象自动递归代理。<br>
